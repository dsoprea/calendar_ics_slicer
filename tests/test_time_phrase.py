...
def test_parse_time_phrase_supports_weeks():
    ...
    interval = calendar_ics_indexer.time_phrase.parse_time_phrase("10 weeks")
...
def test_subtract_time_phrase_moves_instant_backward():
    ...
    cutoff_instant = calendar_ics_indexer.time_phrase.subtract_time_phrase(
        reference_instant,
        "2 days",
    )
...
def test_parse_time_phrase_rejects_unknown_unit():
    ...
    try:
        calendar_ics_indexer.time_phrase.parse_time_phrase("3 fortnights")
        raised = False
    except ValueError:
        raised = True
